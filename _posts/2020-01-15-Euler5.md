...
We can iteratively build the list of prime multiples that satisfies the LCM problem. This is because for every composite number $$\mathbb{C} \in \{1,\ldots, N \}$$, there exists prime numbers smaller than the composite number which are factors of the composite.
