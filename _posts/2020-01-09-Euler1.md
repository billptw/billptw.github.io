...
def get_sum(n, i=1, val=0):
    ...
    return val
```
One quick note is the solution is $$\mathcal{O}(N\cdotT)$$ in runtime, where we count up to $$N$$ for each $$T$$ integers. This solution can be very slow if we need to calculate large numbers many times (say, an input of $$\{9999, 10000, 10001\}$$). Is there a way to save previously calculated results?
